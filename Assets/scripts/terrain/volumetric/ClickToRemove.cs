...
{
	public int pingas=1;
	//pingas is the size of the radius of the sphere
	//that the voxels are removed
	private TerrainVolume terrainVolume;


	//this was originally the click to carve script but I wanted to know how to use cubiquity
	//properly so I decided to make this 
	//-gabe feb 27 2014
	...
	void AddVoxels(int xPos, int yPos, int zPos, int range)
	{
		...
		MaterialSet emptyMaterialSet=new MaterialSet(); //I want to replace emptyMaterialSet
		
		// Iterate over every voxel in a cubic region defined by the received position (the center) and
		// the range. It is quite possible that this will be hundreds or even thousands of voxels.
		for(int z=zPos-range;
		    z<zPos+range;
		    z++)
		{
			for(int y=yPos-range;
			    y<yPos+range;
			    y++)
			{
				for(int x=xPos-range;
				    x<xPos+range;
				    x++)
				{
